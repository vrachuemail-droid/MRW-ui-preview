...
export default {
  async fetch(request, env) {
    ...
    const allowedOrigins = String(env.ALLOWED_ORIGINS || ALLOWED_ORIGIN)
    ...
    if (path === "/api/marrow" && request.method === "POST") {
    ...
    if (path === "/api/create-file" && request.method === "POST") {
    ...
    if (path === "/api/cognitive-sync" && request.method === "POST") {
...
function corsHeaders(origin, env) {
  const allowedOrigins = String(env?.ALLOWED_ORIGINS || ALLOWED_ORIGIN)
    .split(",").map(x => x.trim()).filter(Boolean);
  return {
    "Access-Control-Allow-Origin": origin && allowedOrigins.includes(origin) ? origin : allowedOrigins[0] || ALLOWED_ORIGIN,
...
async function handleAttachmentUpload(request, env, cors) {
  ...
  if(!file.size || file.size>MAX_ATTACHMENT_BYTES) return json({error:"File is too large. Maximum is 50 MB."},413,cors);
  try{
...
async function handleMarrow(request, env, cors) {
  ...
  const requestedAttachmentIds = Array.isArray(body?.attachments) ? body.attachments : [];
  if (!messages.length && requestedAttachmentIds.length) {
    messages = [{ role: "user", parts: [{ text: "Please analyze the attached file(s)." }] }];
  }
